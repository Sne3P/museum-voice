...
/**
 * POST - Upload ou mise à jour d'une image pour une œuvre
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `artwork_${cleanArtworkId}_${timestamp}.${ext}`
    const relativePath = `/uploads/artworks/${fileName}`
    const fullPath = path.join(process.cwd(), 'public', 'uploads', 'artworks', fileName)
    ...
    console.error('Erreur POST artwork-image:', error)
    return NextResponse.json({ 
...
/**
 * DELETE - Suppression d'une image
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Erreur DELETE artwork-image:', error)
    return NextResponse.json({ 
