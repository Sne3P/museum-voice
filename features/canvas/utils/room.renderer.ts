...
export function drawRoomVertices(
  ...
  canvasPoints.forEach((point, index) => {
    ...
    const radius = isHovered ? 8 : isSelected ? 6 : 5

    // Ombre pour 3D effect
    ...
    // Numéro du vertex en mode debug/précis
    if (isHovered || isSelected) {
      ctx.fillStyle = '#000000'
      ctx.font = 'bold 10px monospace'
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      ctx.fillText(`${index}`, point.x, point.y - radius - 8)
      
      // Coordonnées exactes
      const worldPoint = room.polygon[index]
