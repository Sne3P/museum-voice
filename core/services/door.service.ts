...
export function validateDoorMove(
  ...
  // Vérifier la largeur
  // door.segment est en PIXELS (coordonnées monde)
  // Conversion: 40px = 0.5m → 1px = 0.0125m → 80px = 1m
  const doorWidthPixels = Math.sqrt(
    Math.pow(door.segment[1].x - door.segment[0].x, 2) +
    Math.pow(door.segment[1].y - door.segment[0].y, 2)
  )
  
  // Conversion pixels vers mètres: 80px = 1m
